...
const CarouselController: React.FC<ControllerProps> = ({
 ...
 inView
  ...
  const scrollToElement = (id: string, index: number) => {
    const [currentElement, setCurrentElement] = useState(index);
    const prevIndex = usePrevious(currentElement);
  
    const inView = true; // assuming `inView` is always true for this example
