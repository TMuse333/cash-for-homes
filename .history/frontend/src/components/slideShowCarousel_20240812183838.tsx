...
function resetSlideShow(){
  ...
  setSlideProgress(0);
  scrollToElement(`carousel-element-${0}`, 25);
}

useEffect(()=> {
  setSlideProgress(0);
  setSlideProgressReset(true);
  setPrevIndex(currentElement)
...
useEffect(() => {
  ...
  return (
      ...
      bottom-0 z-[2000]'>
          ...
          md:scale-[1.5]'>
            {showRefreshBar ? (
               <FaRedo className="icon ml-auto mr-auto md:scale-[1.5] " onClick={resetSlideShow} />
            ) : slideShowPaused ?  (
              <FaPlay className="icon ml-auto mr-auto md:scale-[1.5]" onClick={togglePlay}/>
            ) : (
              <FaPause className="icon ml-auto mr-auto md:scale-[1.5]" onClick={togglePlay} />
            )}
          </button>
      </div>
  );
...
className='absolute z-[14] text-white w-2/5 text-sm sm:text-lg md:text-4xl top-[5%] left-[15%] md:left-[20%]'>{description}</motion.p>
</AnimatePresence>
            ...
            id={`carousel-element-${index}`}
                ...
                h-full max-w-[1200px] max-h-[] overflow-y-hidden
               `}
