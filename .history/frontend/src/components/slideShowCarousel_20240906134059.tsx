...
useEffect(() => {
  ...
  }, 1000); // Interval of 250ms for smoother transition
...
<AnimatePresence>
  ...
  <motion.p
    ...
    animate={{
      ...
      transform: isCurrentSlide && isMobile ? 'translate(0%,10%)' : isCurrentSlide ? 'translate(15%,10%)' : 'translate(20%,-50%)', // Use transform for movement
      transition: {
    ...
    className='absolute z-[14] text-white w-[80%] max-w-[600px] text-md sm:text-lg md:text-4xl'
    style={{
...
const SlideShowCarousel: React.FC<CarouselProps> = ({ images, title, description }) => {
    ...
    return (
        ...
        overflow-x-hidden  pb-14  h-[105vw] 
        md:max-h-[800px]
        '>
          <div 
