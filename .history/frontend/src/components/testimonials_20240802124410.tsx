...
const Testimonials:React.FC<TestimonialProps> = (
    ...
    return (<>
    <section className='bg-gradient from gray-800 to black border border-white 
    ml-auto mr-auto max-w-[1200px] w-[80vw] relative'>
        <button className='absolute top-1/2 right-0'>Next</button>
        <article className='w-[80%] ml-auto mr-auto'>

    
        <p className='text-center mb-4 mt-2 text-3xl'>{temp.title}</p>
        <p className=' ml-auto mr-auto mb-4'>{temp.description}</p>
        <p className='mb-4'>-{temp.name}</p>
        </article>
