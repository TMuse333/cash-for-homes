...
const CarouselController: React.FC<ControllerProps> = ({
 ...
 inView
  ...
  const scrollToElement = (id: string, index: number) => {
    ...
    if (element) {
      const container = element.parentElement; // Assuming the parent is the scrollable container
      if (container) {
...
useEffect(()=> {
  ...
  if(document.getElementById(`carousel-element-${0}`)){
    document.getElementById(`carousel-element-${0}`).scrollLeft += 20;
  }
